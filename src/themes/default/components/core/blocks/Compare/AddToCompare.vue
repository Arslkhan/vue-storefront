  ...
  methods: {
    addProduct (product) {
      ...
      this.$store.dispatch('notification/spawnNotification', {
        type: 'success',
        message: i18n.t('Product ' + htmlDecode(product.name) + ' has been added to the compare!', { productName: htmlDecode(product.name) }),
    ...
    removeProduct (product) {
      ...
      this.$store.dispatch('notification/spawnNotification', {
        type: 'success',
        message: i18n.t('Product ' + htmlDecode(product.name) + ' has been removed from compare!', { productName: htmlDecode(product.name) }),
        action1: { label: i18n.t('OK') }
      }, { root: true })
    }
  }
